...
public class Inventory
{
	...
	CharacterStats _owner;
	int _maxItems;

	public Inventory(CharacterStats owner = null, int maxItems = 0) { _owner = owner; _maxItems = maxItems; }

	public CharacterStats Owner => _owner;
	...
	{
		if (amount <= 0) return;
		if (_maxItems > 0 && !_items.ContainsKey(itemId) && _items.Count >= _maxItems) return;
	...
	public string TransferTo(Inventory target, string itemId, int amount)
	{
		if (target == null) return "目标背包不存在";
		if (target == this) return "不能转移到自身的背包";
		if (amount <= 0) return "数量无效";
		if (!HasItem(itemId) || GetCount(itemId) < amount) return "物品不足";
		if (target.MaxItems > 0 && !target._items.ContainsKey(itemId) && target.ItemCount >= target.MaxItems) return "目标背包已满";
	...
	public string UseItem(string itemId) => UseItemOn(itemId, _owner);
	...
	{
		if (_owner == null) return "该角色无法装备物品";
		var def = ItemDef.Get(itemId);
		if (def == null) return "未知物品";
		...
		if (_equipped.ContainsKey(def.Slot))
			UnequipItem(def.Slot);

		foreach (var (key, val) in def.Modifiers)
			_owner.ApplyModifier(key, val);
